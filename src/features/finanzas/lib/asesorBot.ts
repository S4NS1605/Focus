...
export const detectarMovimiento = (
  ...
  if (!isQuestion && !context._isRecursive) {
    ...
    if (partsWithNumbers.length > 1) {
      ...
      let combinedText =
        '¡Guau, a la velocidad de la luz! Detecté varias transacciones de una sola pasada:\n\n';

      let lastKind = 'gasto'; // Default propagation
      for (const part of partsWithNumbers) {
        const partToParse =
          part.includes('gaste') || part.includes('pague') || part.includes('me pagaron')
            ? part
            : (lastKind === 'ingreso' ? 'me pagaron ' : 'gaste ') + part;

        const subIntent = parseTransaction(
          partToParse,
          cuentasParaElegir,
          categorias,
          lexico,
  ...
  const isWeakAmount =
    intent.amount !== null &&
    intent.amount < 100 &&
    intent.signals.amountSource === 'words' &&
    !norm.includes('peso') &&
    !norm.includes('dolar') &&
    !norm.includes('euro');
...
export function responderAsesor(
  ...
  if (
    ...
    if (parts.length === 2 && parts[0].length > 6 && parts[1].length > 6) {
      ...
      const p2 = parts[1].includes('cuanto')
        ? parts[1]
        : parts[0].includes('cuanto')
          ? 'cuanto ' + parts[1]
          : parts[1];
  ...
  if (
    context.ultimoAsunto &&
    (norm.startsWith('fueron ') ||
      norm.startsWith('era ') ||
      norm.startsWith('eran ') ||
      norm.includes('corrijo') ||
      norm.includes('ah no,'))
  ) {
  ...
  if (
    ...
    norm.includes('como voy') ||
    norm.includes('como me ves') ||
    norm.includes('como ves') ||
    norm.includes('finanzas') ||
    norm.includes('gastando mucho') ||
    norm.includes('analisis')
  ) {
    const today = new Date();
    const currentMonth = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}`;
    const mesTxs = transacciones.filter((t) => t.occurredOn.startsWith(currentMonth));

    const ingresos = mesTxs
    ...
    if (ahorro > 0) {
      ...
      if (Number(tasaAhorro) > 20) {
        text +=
          'Ahorrar más del 20% es una tasa muy sólida. Sigue así y vas a construir un colchón fuerte.';
      } else {
        text +=
          'Vas bien, en verde. Intenta no subir los gastos de aquí a fin de mes para mantener ese ahorro.';
      }
      ...
      text +=
        'Estás gastando más de lo que ha entrado este mes. Toca apretarse el cinturón o revisar si te faltó registrar algún ingreso.';
    }
    ...
    // Biggest Transaction
    const biggestTx = mesTxs
      .filter((t) => t.kind === 'gasto')
      .sort((a, b) => b.amountCop - a.amountCop)[0];
    ...
    const suggestions = topCat
      ? [`¿Cuánto he gastado en ${topCat[0]}?`, 'Dame un consejo']
      : ['Dime mi saldo', 'Dame un consejo'];
  ...
  if (
    ...
    if (cuentaEspecifica) {
      ...
      return {
        text: `En tu cuenta **${cuentaEspecifica.nombre}** tienes un saldo de $${bal.toLocaleString('es-CO')}.`,
        newContext,
      };
    }
    ...
    return {
      ...
      newContext,
      suggestions:
        totalCuentas > 1000000 && totalAhorro < 100000
          ? ['¿Cómo crear una Cajita?', 'Dime mi resumen']
          : ['Dime mi resumen', '¿Cuánto puedo gastar?'],
    };
  }
  ...
  if (
    norm.includes('puedo gastar') ||
    (norm.includes('cuanto') && norm.includes('me queda')) ||
    ...
    if (totalCuentas === 0) {
      return {
        text: 'No tienes saldo en tus cuentas ahora mismo para calcular un presupuesto.',
        newContext,
      };
    }
  ...
  if (
    norm.includes('suscripciones') ||
    norm.includes('gastos fijos') ||
    norm.includes('pagos recurrentes')
  ) {
    const counts: Record<string, { occurrences: number; amount: number; lastDate: string }> = {};

    // Agrupar por descripción exacta
    transacciones
      .filter((t) => t.kind === 'gasto' && t.description.length > 3)
      .forEach((t) => {
    ...
    const recurrentes = Object.entries(counts)
      .filter(([_, v]) => v.occurrences >= 2)
      .sort((a, b) => b[1].amount - a[1].amount);

    if (recurrentes.length === 0) {
      return {
        text: 'No he detectado suscripciones ni gastos fijos que se repitan en tu historial por ahora.',
        newContext,
      };
    }
  ...
  if (norm.match(/sorpren.*deme|dato curioso|raro|interesante|sorprendeme/)) {
    if (transacciones.length < 10) {
      return {
        text: 'Aún no tienes suficientes transacciones para que mi motor analítico encuentre patrones interesantes. ¡Registra más gastos!',
        newContext,
      };
    }
    ...
    gastos.forEach((t) => {
      descFreq[t.description] = (descFreq[t.description] || 0) + 1;
    });
    ...
    insights.push(
      `**Tu día más caro:** históricamente, el día de la semana en el que más dinero gastas es el **${dayName}**. Considera dejar la tarjeta en casa ese día.`,
    );

    if (anomaly) {
      insights.push(
        `**Gasto inusual:** el ${anomaly.occurredOn} gastaste **$${anomaly.amountCop.toLocaleString('es-CO')}** en "${anomaly.description}" — muy por encima de tu promedio normal por compra ($${Math.round(avg).toLocaleString('es-CO')}).`,
      );
    }

    if (topFreq && topFreq[1] > 3) {
      insights.push(
        `**Frecuencia constante:** has pagado por "${topFreq[0]}" un total de **${topFreq[1]} veces**. Ese es tu gasto hormiga más silencioso.`,
      );
    }

    if (top2Freq && top2Freq[1] > 2) {
      insights.push(
        `**Otro gasto frecuente:** "${top2Freq[0]}" se repite mucho en tu historial (lo has pagado ${top2Freq[1]} veces). Si lo recortas a la mitad, verás crecer tu ahorro.`,
      );
    }
  ...
  if (
    ...
    !intent.amount &&
    (norm.includes('gaste') ||
      norm.includes('compre') ||
      norm.includes('pague') ||
      norm.includes('costo') ||
      norm.includes('cobraron') ||
      norm.includes('me pagaron') ||
      norm.match(/fui a (comer|cenar|comprar|mercar)/))
  ) {
    return {
  ...
  if (isQuestion || norm.includes('gasto') || norm.includes('gaste') || norm.includes('gastos')) {
    ...
    const RELLENO_PREGUNTA = new Set([
      'este',
      'esta',
      'el',
      'la',
    ...
    const isMeaningfulDescription = Boolean(
      ...
      !normalizarNombre(intent.description)
        .split(' ')
        .every((palabra) => RELLENO_PREGUNTA.has(palabra)),
    );

    if (intent.signals.categorySource !== 'default' || isMeaningfulDescription || fuzzyCat) {
      const catFinal =
        intent.signals.categorySource !== 'default' ? intent.category : fuzzyCat || intent.category;
        ...
        (t) =>
          ...
          (isMeaningfulDescription &&
            (normalizarNombre(t.description).includes(normalizarNombre(intent.description)) ||
              (t.rawTranscript &&
                normalizarNombre(t.rawTranscript).includes(
                  normalizarNombre(intent.description),
                )))) ||
          (fuzzyCat && t.category === fuzzyCat),
      );
      asunto =
        intent.signals.categorySource !== 'default'
          ? intent.category
          : fuzzyCat || intent.description || 'varios';
    ...
    const tipo =
      norm.includes('ingreso') || norm.includes('gane') || norm.includes('recibi')
        ? 'ingreso'
        : 'gasto';
    ...
    let fechaStr = filterDate
      ? filterDate.length === 7
        ? `en el mes de ${filterDate}`
        : filterDate.length === 4
          ? `en el año ${filterDate}`
          : `el ${filterDate}`
      : 'este mes';
    if (norm.includes('año pasado')) fechaStr = 'el año pasado';
    else if (norm.includes('mes pasado') || norm.includes('mes anterior'))
      fechaStr = 'el mes pasado';
    ...
    if (filtered.length === 0) {
      ...
      return {
        text: getRandom(varsCero)
          .replace('X', total.toLocaleString('es-CO'))
          .replace('Y', concepto)
          .replace('Z', fechaStr),
        newContext,
      };
    ...
    return {
      text:
        getRandom(varsFinal)
          ...
          .replace('Y', concepto)
          .replace('Z', fechaStr) +
        ` (Eso fue en ${filtered.length} transacciones). ${tendencia}` +
        drilldown,
      newContext,
      suggestions:
        topContacto && topContacto[0] !== 'Sin descripción'
          ? [`¿Y el mes pasado?`, `¿Cuánto puedo gastar?`]
          : ['Dime mi resumen', 'Dame un consejo'],
    };
  }
  ...
  if (norm.length > 30 && !norm.includes('gaste') && !norm.includes('gastado')) {
    return {
      text: '¡Esa es una pregunta profunda! Como soy una IA local basada en reglas, soy experto en buscar sumas, fechas y gastos exactos, pero me cuesta leer párrafos muy largos. Prueba preguntándome montos o resúmenes directos.',
      newContext,
    };
  }
  ...
  const chitchatRules: Array<{ regex: RegExp; responses: string[] }> = [
    ...
    {
      regex:
        /(como|puedo) (borrar|eliminar|editar|modificar|corregir|cancelar|deshacer)|me equivoque/i,
      responses: [
