...
const Progreso: React.FC<{ progreso: ProgresoMeta }> = ({ progreso }) => (
  <>
    <div className="mt-3 h-2.5 overflow-hidden rounded-[var(--fin-r-pill)] bg-[var(--fin-soft)]">
      <div
        className="h-full rounded-[var(--fin-r-pill)] transition-[width] duration-500"
        style={{
          width: `${progreso.pct}%`,
    ...
    <div className="mt-2 flex items-baseline justify-between gap-2">
      <span className="text-[13px] font-semibold tabular-nums text-[var(--fin-ink)]">
        {formatCop(progreso.ahorradoCop)}
        <span className="font-normal text-[var(--fin-ink-faint)]">
          {' '}
          / {formatCop(progreso.objetivoCop)}
        </span>
      </span>
      <span className="text-[13px] font-semibold tabular-nums text-[var(--fin-ink-soft)]">
        {progreso.pct}%
      </span>
    </div>
  </>
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-5">
          ...
          <form onSubmit={crear} className="rounded-[var(--fin-r-card)] bg-[var(--fin-card)] p-5">
            ...
            <div className="mt-2 flex items-center gap-2 rounded-[var(--fin-r-card)] border-2 border-[var(--fin-line)] bg-[var(--fin-card)] px-4 py-3">
              ...
              <input
                ...
                value={objetivoTexto}
                onChange={(e) =>
                  setObjetivoTexto(formatAmountInput(parseAmountInput(e.target.value)))
                }
                inputMode="numeric"
                placeholder="0"
        ...
        {filas.length === 0 && !creando ? (
          <div className="rounded-[var(--fin-r-card)] border-2 border-dashed border-[var(--fin-line)] px-6 py-12 text-center flex flex-col items-center">
            ...
            <p className="mt-1 text-[15px] text-[var(--fin-ink-faint)]">{COPY.metas.vacioHint}</p>
          </div>
        ) : null}
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <span
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[var(--fin-r-card)] bg-[var(--fin-soft)]"
                    aria-hidden="true"
                  >
                    {(() => {
                ...
                <div className="mt-3 rounded-[var(--fin-r-card)] bg-[var(--fin-bg)] px-4 py-3">
                  {progreso.completada ? (
                    <p className="text-[15px] font-semibold text-[var(--fin-in)]">
                      {(() => {
                ...
                {meta.cajitaId === null ? (
                  editandoAhorro === meta.id ? (
                    <div className="mt-3 flex items-center gap-2">
                      <div className="flex flex-1 items-center gap-2 rounded-[var(--fin-r-card)] border-2 border-[var(--fin-line)] bg-[var(--fin-card)] px-3 py-2">
                        <span className="text-[20px] font-semibold text-[var(--fin-ink-faint)]">
                          $
                        </span>
                        <input
                          value={ahorroTexto}
                    ...
                    <button
                      ...
                      className="mt-3 w-full rounded-[var(--fin-r-pill)] bg-[var(--fin-soft)] px-4 py-2.5 text-[15px] font-semibold text-[var(--fin-ink-soft)] transition-colors hover:text-[var(--fin-ink)]"
                    >
